...
/**
 * Package private as util to convert between URI and SSHLinuxPath
 * @author wangho
 *
 */
class SSHLinuxUriUtils {
    private SSHLinuxUriUtils() { }

    /**
     * Converts URI to Path
     */
    static Path fromUri(SSHLinuxFileSystem fs, URI uri) {
        if (!uri.isAbsolute())
        ...
        return new SSHLinuxPath(fs, result);
    }

    /**
     * Converts Path to URI
     */
    static URI toUri(SSHLinuxPath up) {
        byte[] path = ((SSHLinuxPath) up.toAbsolutePath()).asByteArray();
        ...
        if (sb.charAt(sb.length()-1) != '/') {
            try {
                 if ((up.getFileSystem() instanceof SSHLinuxFileSystem)
                		 && ((SSHLinuxFileSystem)up.getFileSystem()).getFileAttributes(up,false).isDirectory())
                     sb.append('/');
            } catch (Exception x) {
                // ignore
            }
